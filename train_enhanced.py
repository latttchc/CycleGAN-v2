...
def train_fn(disc_A, disc_B, gen_B, gen_A, loader, opt_disc, opt_gen, 
    ...
    Args:
        disc_A, disc_B: Discriminator
        gen_B, gen_A: Generator
        loader: DataLoader
        opt_disc, opt_gen: Optimizer
        l1, mse: 基本損失関数
        color_loss: 色恒常性損失
        perceptual_loss: 知覚損失
        contrast_loss: コントラスト正則化
        d_scaler, g_scaler: GradScaler
    ...
    for idx, (trainB, trainA) in enumerate(loop):
        ...
        with torch.cuda.amp.autocast():
            ...
            # ========== 拡張損失 ==========
            # 1. 色恒常性損失（白内障による色の偏りを補正）
            cc_loss_A = color_loss(fake_trainA, trainA)
            cc_loss_B = color_loss(fake_trainB, trainB)
            ...
            G_loss = (
                ...
                total_cc_loss +           # 色恒常性
                total_perc_loss +         # 知覚損失
                total_contr_loss          # コントラスト正則化
            )
        ...
        total_perceptual_loss += total_perc_loss.item()
        total_contrast_loss += total_contr_loss.item()
        ...
        loop.set_postfix(
            ...
            Perc=total_perceptual_loss / (idx + 1)
        )
    ...
    return {
        ...
        'B_fakes_score': B_fakes / num_batches,
    }
...
def main():
    ...
    """
    print("=" * 50)
    print("Enhanced CycleGAN Training Started")
    ...
    print(f"  - ContrastRegularization (λ={config.LAMBDA_CONTRAST})")
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        if config.SAVE_HISTORY or config.SAVE_PROGRESS:
            ...
            print(f"  Color_Loss: {epoch_metrics['color_loss']:.4f}")
            print(f"  Perceptual_Loss: {epoch_metrics['perceptual_loss']:.4f}")
            print(f"  Contrast_Loss: {epoch_metrics['contrast_loss']:.4f}")
